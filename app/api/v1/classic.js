...
router.get('/latest', new Auth().m, async(ctx, next) => {
    ...
    // 如果都过了 就返回ctx
    // 需要category+artId
        if (!ctx.query.uid) throw new ParameterException('uid必填')
        ...
        const ifUserLiked = await Art.getIfLike(ctx.query.uid, flow.art_id, flow.type) 
...
router.post('/likeArt', new Auth().m, async(ctx, next) => {
    ...
    await Like.like(data.uid, data.art_id, data.type)
    ctx.body = {
        data: 'ur good to go'
    }
})
...
router.post('/dislikeArt', new Auth().m, async(ctx, next) => {
    ...
    const v = await new LikeValidator().validate(ctx)  // 如果没问题 这里开始组合一条Like数据 并插入Like表
    const data = v.get('body')
    await Like.dislike(data.uid, data.art_id, data.type)
    ctx.body = {
        data: '已删除'
    }
})
